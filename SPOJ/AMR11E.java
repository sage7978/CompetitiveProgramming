...
// URL: https://www.spoj.com/problems/AMR11E/
	...
	void solve() throws Exception {
		...
		for(int i=2;i<N;i++){
			if(primeFactor[i] == -1){
				...
				// p(current + " ");
				while(current > 1){
					int factor = primeFactor[current];
					if(factor == -1){
						factor = current;
					}			
					// p("[" + current + "," + distinct + "," + factor + "]");
				...
				primeFactorCounts[i] = distinct;
				// pn(" dj : " + i + " " + primeFactorCounts[i]);
			}
			if(primeFactorCounts[i] >= 3){
				x.add(i);
			}
		}	
